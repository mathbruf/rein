...
- **`IDEAS.md`** — backlog of proposed improvements + the "what lies ahead" review.
...
The model is portable to other areas: the area seam lives in `config/area.json`
(+ `REINDEER_AREA_CONFIG` override).
...
```
**Everything you look at lands in one place — `output/`** (reproducible):
```
output/
...
`data/` holds only pipeline inputs/intermediates; the validation scripts write
their full reports into `docs/` on each run.

**What is public vs local:** the repository publishes **no data** — no field
reports, positions, gazetteer or boundary geometry, and no forecast or
historical maps. The published artifacts are the **aggregate analysis charts**
in [`output/analysis/`](output/analysis/) (+ their README) — each figure
carries the project identity, data credits and honest limits — and the fixed
**demo-scenario images** in [`output/demo/`](output/demo/) that show the
regime flip on synthetic weather days. Everything else in `output/` stays on
the local machine.
...
| | |
|---|---|
| ![hit-rate](output/analysis/hit_analysis.png) | ![breakdown](output/analysis/validation_breakdown.png) |
| ![drivers](output/analysis/weather_drivers.png) | ![explainer](output/analysis/model_explainer.png) |
...
**Honest validation result:** tested
...
trusted. See `output/analysis/hit_analysis.png` and `output/analysis/README.md`.
